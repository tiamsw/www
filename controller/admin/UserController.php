...
class UserController extends Controller {
	public function index() {
		...
		$this->smarty->display ( "admin/users/user_list.tpl" );
	}
	...
	function getgridpage() {
		...
		for($i = 0; $i < $num; ++ $i) {
    		  ...
    		  $responce->rows[$i]->cell=array($row->id,$row->user_name,$row->real_name,$row->mobile,$row->email,$row->login_time,$row->login_ip,$row->user_desc);
		}
