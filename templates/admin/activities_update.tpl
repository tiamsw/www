...
function checkform(){
	...
	$(".imgname").each(function(){ 
     ...
     if(obj.defaultimgs == null){
	    ...
	    $.post(
				'{{$smarty.const.WEBSITE_URL}}admin/activities/update',
				obj,
				function(data){ 
					 alert("上传成功");
				},
	...
	<div id="myTabContent" class="tab-content">
		  <div class="tab-pane active in" id="home">
           ...
           <form id="tab" method="post" action="" autocomplete="off" onsubmit="javascript: return false;">
				<label>活动名称</label>
				<input type="text" name="activities_name" value="{{$activity->acti_name}}"  id = "activities_name" class="input-xlarge" autofocus="true" required="true" >
				<label>活动类型</label>  
				 <select name="activities_type" id="activities_type">
				  <option value="1" {{if $activity->acti_type == 1}}selected{{/if}} >图片</option>
				  <option value="2"  {{if $activity->acti_type == 2}}selected{{/if}}>视频</option>
				 </select> 
				 <label>文件</label>  
				  <p id="errorRemind"></p>
                  <input id="unloadPic" type="button" value="上传图片" />
                    <input type="hidden" value="{{$activity->id}}" id="ac_id" />
				<table id="uploadedName" class="table table-hover" width="51%"> 
				<tr><th>默认显示</th><th>文件名</th><th>操作</th></tr>
				  {{foreach $activities as $key=>$activity}}
					  <tr id='{{$key}}'><td><input type='radio' name='imgdefault'  value='{{$activity->imgname}}'   {{if $activity->f_id == $defaultid}}checked{{/if}}/></td>
					  <td> <span class='imgname'>{{$activity->imgname}}</span></td>
					  <td><a href="javascript:delrows('{{$key}}');">删除</a></td>
					  </tr>
				  {{/foreach}}
                </table>
				   
				<label>活动描述</label>
				<textarea name="activities_desc" id="activities_desc" rows="6" class="input-xxlarge">{{$activity->acti_desc}}</textarea>
